...
/** Generic, JSON-shaped tagged value.
  *
  * Replaces the role previously played by `zio.json.ast.Json` after the migration to jsoniter-scala. Each variant carries its own `LMDBCodecJson` (derived from the case class definition), so collections can be narrowly typed: `[String, StringV]` for
  * string-only, `[String, LongV]` for integer-only, etc. A `[String, JValue]` collection accepts any variant via the sum-type codec at the bottom of this file.
  *
  * Graph-native variants (`InstantV`, `IdentifierV`) live alongside the JSON primitives so the upcoming L3 layer can express typed property bags without a second hierarchy. L5 will add `IRIV` as an additive case when the semantic overlay loads. See
  * `docs/internal/EVOLUTION_PLAN_ITERATION_6.md` §6.5 for the design rationale.
  *
  * ==Why the sum-type codec is hand-rolled==
  *
  * `JsonCodecMaker.make[JValue]` (with `withAllowRecursiveTypes(true)`) hits a forward-reference emit-order bug in jsoniter-scala 2.38 whenever the variant set contains both recursive cases (`ListV[JValue]`, `MapV[String, JValue]`) AND more than one
  * "special" string-encoded type (`Instant`, `UUID`, `BigDecimal`). Bisection showed each subset works alone, but the full set provokes `d0 is a forward reference extending over the definition of cN`.
  *
  ...
  *   - Scala 3.8.4-RC2
  *
  * A hand-rolled `JsonValueCodec[JValue]` sidesteps the macro entirely and keeps the same `{"type":"VariantName","value":...}` wire format the macro would have emitted. Per-variant case classes still use `derives LMDBCodecJson` for narrowly-typed
  * collections; only the sum-type entry point is hand-rolled.
  *
  * '''Future check.''' When upgrading Scala or jsoniter-scala, re-test by swapping the `JValueValueCodec` definition below for the one-liner kept as `// MACRO-FUTURE-CHECK` comments after it. If `core / compile` succeeds, the hand-rolled object and
  * its imports (`JsonReader`, `JsonWriter`, `ListMap`, `mutable`) can be deleted in favour of the macro derivation. `JValueCodecSpec` covers the wire format and round-trip surface either way.
  */
sealed trait JValue
...
object JValue {
  ...
  /** Arbitrary-precision decimal — useful when the application cares about exact decimal arithmetic (money, scientific data). Round-trip preserves precision via jsoniter-scala's `BigDecimal` codec.
    */
  ...
  /** A JSON object — string-keyed bag of values. The recursive shape supports nested property bags, e.g. an L3 node whose property is itself a sub-document.
    */
  ...
  /** Raw jsoniter codec exposed in implicit scope so that `JsonCodecMaker.make[T]` reuses it for any `T` that has a `JValue` field, instead of inlining `JValue`'s recursive structure (which would re-trigger the macro forward-reference bug documented
    * above). This is jsoniter-scala's documented "implicitly accessible codec" escape hatch for recursion.
    */
  ...
  /** Parse plain JSON bytes (as written by a value codec) into a generic [[JValue]] tree. Integral numbers become `LongV`, fractional numbers `DecimalV` (precision-preserving). Used by the L2C SQL engine to read stored values without their static
    * type.
    */

  ...
  def spec = suite("GEOTools spec")(
    test("roundtrip conversion") {
      check(Gen.double(-90.0, 90.0), Gen.double(-180.0, 180.0)) { (lat, lon) =>
        ...
        assert(latError)(isLessThan(1.0e-7)) &&
        assert(lonError)(isLessThan(1.0e-7))
      }
    },
    ...
    test("masking example") {
      ...
      // defined by the mask.
      assertTrue(regionLoc.latitude <= loc.latitude) &&
      assertTrue(regionLoc.longitude <= loc.longitude)
    }
  )
}
...
## 3.0 - 2026-06-09 release candidate cyles
...
- Add string-manipulation functions to the SQL layer: `UPPER`/`LOWER`, `TRIM`/`LTRIM`/`RTRIM`, `SUBSTR`/`SUBSTRING(s, start[, len])` (1-based), `CONCAT(a, b, …)`, `REPLACE(s, from, to)`, and `INSTR(s, sub)` (1-based index, `0` if absent). Usable anywhere an expression is allowed; non-string arguments are rendered to text, a `NULL` argument yields `NULL`
- Add date/time support to the SQL layer: extraction functions `YEAR`/`MONTH`/`DAY`/`HOUR`/`MINUTE`/`SECOND(ts)` (UTC calendar fields), `DATE_DIFF(unit, a, b)` (signed `a - b` in `second`/`minute`/`hour`/`day`/`millisecond`), and `NOW()`. Timestamps (stored as ISO-8601 text) are parsed leniently (date / date-time / offset date-time) and compare correctly against ISO literals and against `NOW()`/date-function results. Group and order by calendar fields — e.g. `SELECT YEAR(timestamp) AS year, COUNT(*) FROM medias GROUP BY year ORDER BY year`
- Generalize SQL `GROUP BY` and `ORDER BY` to arbitrary expressions: `GROUP BY` accepts any expression (a function call, arithmetic, a nested path) or a `SELECT` alias (e.g. `GROUP BY YEAR(timestamp)` or `GROUP BY year`), and `ORDER BY` takes several comma-separated keys, each with its own `ASC`/`DESC`. Aggregate-query projections may now also be expressions over aggregates (e.g. `SUM(amount) / COUNT(*) AS mean`); every non-aggregated reference must be covered by `GROUP BY`
- Add `IN`/`NOT IN` (list membership), `BETWEEN`/`NOT BETWEEN` (inclusive range), and `NOT LIKE` predicates to the SQL `WHERE`/`HAVING` grammar. List items and range bounds may be arbitrary expressions; a `NULL` left-hand side matches nothing (so it is excluded by the `NOT` forms too)
- Add `CASE` expressions to the SQL layer, both the searched form (`CASE WHEN <cond> THEN <r> … ELSE <r> END`) and the simple form (`CASE <subject> WHEN <v> THEN <r> … END`, comparing the subject for equality). The first matching branch wins; with no match the result is the `ELSE` value or `NULL`. Usable anywhere an expression is allowed — projection, `WHERE`, `HAVING`, `ORDER BY`, and as a `GROUP BY` bucket
- Add null-handling and conversion functions to the SQL layer: `COALESCE(a, b, …)` (first non-`NULL` argument), `NULLIF(a, b)` (`NULL` when `a` equals `b`), and `CAST(x AS <type>)` converting to `integer`/`double`/`decimal`/`string`/`boolean`/`timestamp` (with common aliases). Numeric parsing is lenient and a `NULL` or unconvertible value yields `NULL`
- Generalize SQL aggregate arguments to arbitrary expressions and add `DISTINCT`: `SUM`/`AVG`/`MIN`/`MAX`/`COUNT` now take any scalar expression (e.g. `SUM(price * qty)`, `MAX(YEAR(timestamp))`), and a leading `DISTINCT` dedupes the argument values first (`COUNT(DISTINCT customer)`, `SUM(DISTINCT amount)`). `COUNT(*)` is unchanged
- Add numeric functions to the SQL layer: `ABS`, `FLOOR`, `CEIL`/`CEILING`, `ROUND(x[, n])`, `SIGN`, `MOD(a, b)`, `POWER`/`POW`, and `SQRT`. A non-numeric argument yields `NULL` (and `MOD` by zero / `SQRT` of a negative also yield `NULL`)
- Add non-interactive execution to the SQL REPL: `--execute`/`-e <sql>` (repeatable) runs statements against the named database and exits instead of prompting, with `--format`/`-f table|json|csv` choosing the output. Results go to stdout and logs to stderr (so it pipes cleanly), and a failing statement prints `error: …` to stderr and exits non-zero
- Fix the SQL engine routing a whole-table aggregate to the per-row streaming path when the only aggregate was nested inside an expression projection or `ORDER BY` key (e.g. `SELECT ROUND(AVG(x), 2) FROM t`, `SUM(a) / COUNT(*)`) — such queries now correctly return a single aggregated row

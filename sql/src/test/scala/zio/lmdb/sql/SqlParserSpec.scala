  ...
  val spec = suite("SqlParser")(
    ...
    test("COUNT(*) and COUNT(col) projections; bare 'count' stays a column") {
      assertTrue(
        ...
        ok("SELECT COUNT(*) FROM t WHERE a > 1") == Statement.Select(
          Projection.Items(List(SelectItem.Agg(AggFunc.Count, None))),
          false,
          "t",
          Some(Expr.Cmp(CmpOp.Gt, Expr.Col("a"), Expr.Lit(Literal.IntLit(1)))),
          Nil,
          None,
          Nil,
          None
    ...
    test("aggregates with GROUP BY, MIN/MAX/SUM/AVG, and SELECT DISTINCT") {
      assertTrue(
        ...
        ok("select min(age), max(age) from people")
          == Statement.Select(
            Projection.Items(List(SelectItem.Agg(AggFunc.Min, Some(Expr.Col("age"))), SelectItem.Agg(AggFunc.Max, Some(Expr.Col("age"))))),
            false,
            "people",
            None,
            Nil,
        ...
        ok("select distinct city, country from people order by country limit 5")
          == Statement.Select(
            Projection.Items(List(SelectItem.Col("city"), SelectItem.Col("country"))),
            true,
            "people",
            None,
            Nil,
            None,
            List(OrderBy(Expr.Col("country"), descending = false)),
            Some(5L)
          )
      )
    },
      ...
      assertTrue(
        ok("select year(ts), count(*) from t group by year(ts)")
          == Statement.Select(
            ...
            "t",
            None,
            List(Expr.Func("year", List(Expr.Col("ts")))),
            None,
            Nil,
            None
          ),
      ...
      assertTrue(
        ...
        ok("select * from originals o where o.dimension.width > 1920 order by o.location.altitude desc")
          == Statement.Select(
            ...
            Nil,
            None,
            List(OrderBy(Expr.Col("o.location.altitude"), descending = true)),
            None,
            Some("o")
          )
      )
    ...
    test("geo functions: GEO_DISTANCE in SELECT/WHERE/ORDER BY, GEO_WITHIN as a predicate") {
      val dist =
        Expr.Func(
          "geo_distance",
          List(
            Expr.Col("o.location.latitude"),
            Expr.Col("o.location.longitude"),
            Expr.Lit(Literal.DecLit(BigDecimal("48.8566"))),
            Expr.Lit(Literal.DecLit(BigDecimal("2.3522")))
          )
        )
      assertTrue(
        ok(
          ...
          == Statement.Select(
            ...
            Nil,
            None,
            List(OrderBy(Expr.Col("dist"), descending = false)),
            None,
            Some("o")
          ),
      ...
      assertTrue(
        ...
        ok("select x / 1000 as km from t where x / 1000 <= 10 order by km")
          == Statement.Select(
            ...
            Nil,
            None,
            List(OrderBy(Expr.Col("km"), descending = false)),
            None
          )
      )
    },
      ...
      assertTrue(
        ok("select * from t where age in (25, 40)")
          == Statement.Select(
            ...
            false,
            "t",
            Some(Expr.In(Expr.Col("age"), List(Expr.Lit(Literal.IntLit(25)), Expr.Lit(Literal.IntLit(40))), negated = false)),
            Nil,
            None,
            Nil,
            None
        ...
        ok("select * from t where age not in (25, 40)")
          == Statement.Select(
            ...
            false,
            "t",
            Some(Expr.In(Expr.Col("age"), List(Expr.Lit(Literal.IntLit(25)), Expr.Lit(Literal.IntLit(40))), negated = true)),
            Nil,
            None,
            Nil,
            None
        ...
        ok("select * from t where age between 18 and 65")
          == Statement.Select(
            ...
            false,
            "t",
            Some(Expr.Between(Expr.Col("age"), Expr.Lit(Literal.IntLit(18)), Expr.Lit(Literal.IntLit(65)), negated = false)),
            Nil,
            None,
            Nil,
            None
        ...
        ok("select * from t where age not between 18 and 65")
          == Statement.Select(
            ...
            false,
            "t",
            Some(Expr.Between(Expr.Col("age"), Expr.Lit(Literal.IntLit(18)), Expr.Lit(Literal.IntLit(65)), negated = true)),
            Nil,
            None,
            Nil,
            None
